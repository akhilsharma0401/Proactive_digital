...
export const TracingBeam = ({ children, className, page }) => {
  const ref = useRef(null);
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start start", "end start"],
...
// const y2 = useSpring(
//   useTransform(scrollYProgress, [0, 1], [100, svgHeight - 300]),
//   { stiffness: 500, damping: 90 }
// );
  ...
  return (
    <motion.div ref={ref} className={cn("relative  w-full", className)}>
      <div className=" absolute top-0 -left-5  lg:-left-5 ">
        <motion.div
          transition={{
            duration: 0.2,
            delay: 0.5,
        ...
        >
          <motion.div
            ...
            animate={{
              backgroundColor: scrollYProgress.get() > 0 ? "#3e66f3" : "#3e66f3",
              borderColor: scrollYProgress.get() > 0 ? "#3e66f3" : "#3e66f3",
            }}
            className="h-2 w-2 rounded-full border border-neutral-300 bg-white"
          />
        </motion.div>
      ...
      <div ref={contentRef} className=" ml-5 lg:ml-10 w-full" >{children}</div>
    </motion.div>
  );
};
